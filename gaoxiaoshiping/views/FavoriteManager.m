...
- (instancetype)init
{
    ...
    if (self) {
        ...
        //path:指数据库文件的位置
        NSString *path = [[NSHomeDirectory() stringByAppendingPathComponent:@"Documents"]stringByAppendingPathComponent:@"gaoxiaozan1236.db"];
//        NSLog(@"path%@",path);
        ...
        if (success) {
            //如果打开成功,创建数据库表
            NSString *sql = @"create table if not exists app(applicationid varchar(32))";
...
//    NSLog(@"%@",sql);
    ...
    NSString *sql = @"insert into app (applicationid) values (?)";
    BOOL success = [_fmdb executeUpdate:sql,model];
    if (success) {
      NSLog(@"收藏成功");
    }
}
