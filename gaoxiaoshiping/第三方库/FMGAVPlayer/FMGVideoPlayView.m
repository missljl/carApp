...
@property (nonatomic, strong) juhuaView * juhuaview;
#pragma mark - 监听事件的处理
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"status"]) {
        ...
        if (AVPlayerItemStatusReadyToPlay == status) {
            ...
            [_juhuaview isstop:YES];
            [self removeProgressTimer];
            [self addProgressTimer];
            self.isShowToolView=NO;
//            self.juhuaview.alpha=0;
           NSLog(@"zhahahahahhaha");
...
//
    _juhuaview =[[juhuaView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, self.frame.size.height)];
    
    [_imageView addSubview:_juhuaview];
